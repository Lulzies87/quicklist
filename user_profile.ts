...
document.getElementById(
  "userLevelDisplay",
)!.innerText = `${currentUser.level.number} - ${currentUser.level.Name}\n"${currentUser.level.Description}"`

document.getElementById("userEXPDisplay")!.innerText = `${
  currentUser.xp
} / ${getNextLevelUnlockScore(currentUser.level.number)}`
...
function getNextLevelUnlockScore(currentUserLevelNumber: number) {
